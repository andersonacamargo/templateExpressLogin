...
export const gerar = async (req, res) => {
  ...
  "descricao": "Trilha completa para dominar JavaScript do zero ao avançado, focando em conceitos modernos da linguagem, manipulação do DOM, requisições a APIs e arquitetura de código para o mercado de trabalho.",
  "competencias": [
  ...
  "topicos": [
    ...
    const system =
      ...
      'correta (0-3), topico (string), habilidade (string), difficuldade ("iniciante"|"intermediario"|"avancado"). ' +
      'O JSON raiz deve ter o formato: { "questoes": [...] }.';
    const user = `Gere uma avaliação ${tipo === 'diagnostica' ? 'DIAGNÓSTICA' : 'DE PROGRESSO'} ` +
    ...
    console.log(user)
    const novaAvaliação = await ai.models.generateContent({
      model: 'gemini-3.5-flash', 
      contents: system + user 
      
    });
    console.log(novaAvaliação.text)
    res.status(200).json({
